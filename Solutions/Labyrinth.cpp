...
char ch[1005][1005];
bool vis[1005][1005];
unordered_map<ll, unordered_map<ll, pll> >mp;
...
bool BFS(ll x, ll y)
{
    ...
    while(!q.empty())
    {
        ll tx=q.front().ff,ty=q.front().ss;
        q.pop();

        foR0(4){
            if(tx+fx[i]>=1 && tx+fx[i]<=n && ty+fy[i]>=1 && ty+fy[i]<=m && ch[tx+fx[i]][ty+fy[i]]!='#' && !vis[tx+fx[i]][ty+fy[i]] ){
                vis[tx+fx[i]][ty+fy[i]]=true;
                mp[tx+fx[i]][ty+fy[i]]={tx,ty};
                if(ch[tx+fx[i]][ty+fy[i]]=='B'){
                    check=true;
                    break;
                }
                q.push({tx+fx[i], ty+fy[i]});
            }
        }
        if(check)break;
    }
