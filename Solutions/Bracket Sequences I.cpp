...
int main()
{
    ...
    for(ll tt=1; tt<=tst; tt++)
    {
        ...
        ll fact[2*n+10];
        fact[0]=1;
        foR1(2*n+5)fact[i]=(fact[i-1]*i)%MOD;
        ll down=(fact[n]*fact[n])%MOD;
        ll val1= (fact[2*n]*bigmod(down,MOD-2,MOD))%MOD;
        down=(fact[n+1]*fact[n-1])%MOD;
        ll val2= (fact[2*n]*bigmod(down,MOD-2,MOD))%MOD;
        cout<<(val1-val2+MOD)%MOD<<endl;
