...
int main()
{
    ...
    for(ll tt=1; tt<=tst; tt++)
    {
        ...
        ll fact[n+5];
        fact[0]=1;
        foR1(n)fact[i]=(fact[i-1]*i)%MOD;
        foR1(n)
        {
            ll math = (fact[i]*fact[n-i])%MOD;
            math = bigmod(math,MOD-2,MOD);
            math = (math * fact[n])%MOD;
            math = (math * fact[n-i])%MOD;
            if(i&1)cnt = (cnt+math)%MOD;
            else cnt = (cnt-math+MOD)%MOD;
        }
        ll val=(fact[n]-cnt+MOD)%MOD;
        cout<<val<<endl;
