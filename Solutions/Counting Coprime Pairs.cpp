...
int main()
{
    ...
    for(int tt=1; tt<=tst; tt++)
    {
        ...
        rep(i,0,n)
        {
            rep(j,0,(1<<SZ(prime[A[i]])))
            {
                int cnt=0,mul=1;
                rep(k,0, SZ(prime[A[i]]))
                {
                    if((1<<k)&j)mul*=prime[A[i]][k],cnt++;
                }
                mp[mul]++;
                nmp[mul]=cnt;
            }
        ...
        for(int i=0;i<mxx;i++)
        {
            if(mp[i]==0 || nmp[i]==0)continue;
            ll cnt = mp[i];
            if(nmp[i]&1)res-=(cnt*(cnt-1))/2;
            else res+=(cnt*(cnt-1))/2;
        }
        printf("%lld\n",res);
